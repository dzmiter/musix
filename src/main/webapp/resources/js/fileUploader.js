$(document).ready(function() {
	...
	var z = -40;
	// The number of audios to display
	var errMessage = 0;
	...
	$('#drop-files').bind('drop', function(e) {
		...
		$.each(files, function(index, file) {
			...
			if (!files[index].type.match('audio/*')) {
				...
				if(errMessage == 0) {
					$('#drop-files').html('Hey! Music only');
					++errMessage;
				}
				else if(errMessage == 1) {
					$('#drop-files').html('Stop it! Music only!');
					++errMessage;
				}
				else if(errMessage == 2) {
					$('#drop-files').html("Can't you read?! Music only!");
					++errMessage;
				}
				else if(errMessage == 3) {
					$('#drop-files').html("Fine! Keep dropping non-audio.");
					errMessage = 0;
				}
				return false;
			}			
			...
			var fileReader = new FileReader();
				...
				fileReader.onload = (function(file) {
					...
					return function(e) { 
						...
						// Move each audio 40 more pixels across
						z = z+40;						
						...
						$('#upload-button').css({'left' : '200px', 'display' : 'block'});
	...
	function restartFiles() {
		...
		// We need to remove all the audios and li elements as
	...
	$('#upload-button .upload').click(function() {
		...
		$.each(dataArray, function(index, file) {	
			...
			$.post('upload', dataArray[index], function(data) {
				...
				if(dataSplit[1] == 'uploaded successfully') {
					var realData = '<li><a href="audios/'+dataSplit[0]+'">'+fileName+'</a> '+dataSplit[1]+'</li>';
					
					$('#uploaded-files').append('<li><a href="audios/'+dataSplit[0]+'">'+fileName+'</a> '+dataSplit[1]+'</li>');
					...
					$('#uploaded-files').append('<li><a href="audios/'+data+'. File Name: '+dataArray[index].name+'</li>');
				}
